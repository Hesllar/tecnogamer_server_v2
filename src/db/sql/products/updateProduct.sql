...
AS $BODY$
    BEGIN
		...
		if(not exists(select product_id from products p where p.product_id = p_product_id))then
		...
		if(not exists(select product_id from products p where p.product_id =  p_product_id and p.name_product = p_name_product))then
			if(exists (select product_id from products p1 where p1.name_product = p_name_product))then
